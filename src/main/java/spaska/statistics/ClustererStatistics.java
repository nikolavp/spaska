...
public final class ClustererStatistics extends Statistics {
     ...
     * 
     * @param instances
     *            a map of instances and their cluster number
     */
    ...
    protected void generateInfo() {
        ...
        for (int i = 0; i < clusters.length; i++) {
            ...
            double percent = (PERCENT_BASE * clusters[i])
                    / (double) getInstances();
        ...
        if (getAlgorithmName() != "ZeroClusterer") {
            result.append(getEvaluationInfo());
        }
    ...
    private StringBuilder getEvaluationInfo() {
        ...
        for (int clusterIndex = 0; clusterIndex < clusters.length; clusterIndex++) {
            ...
            Iterator<Entry<Instance, Integer>> iterator = clusteredInstances
                    .entrySet().iterator();
            ...
            while (iterator.hasNext()) {
                Map.Entry<Instance, Integer> pair = (Map.Entry<Instance, Integer>) iterator
                        .next();

                if (pair.getValue() == clusterIndex) {
                    String className = ((NominalValue) service.getClass(pair
                            .getKey())).getValue();
            ...
            for (int classIndex = 0; classIndex < classNames.size(); classIndex++) {
                clusterStatistics.append(instancesInClass[classIndex]
                        + " instances from class " + classNames.get(classIndex)
                        + "\n");
            }
     ...
     * 
     * @param classNames
     *            an array containing all the class names for the testing
     *            dataset
     */
     ...
     * 
     * @param service
     *            the {@link DatasetService} to be used while building the
     *            statistics object
     */
