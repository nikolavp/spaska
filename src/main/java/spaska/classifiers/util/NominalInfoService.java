...
/*Calculates information theory elements for all nominal attributes
 *to support best-informative-attribute decision */
public class NominalInfoService {
				...
				if (val.getType() == ValueType.Nominal) {
					if (statistics[i].get(val) != null) {
						statistics[i].get(val)[datasetService.intValue(
								datasetService.classIndex(), datasetService
										.getClass(instance))]++;
					}
				}
				i++;
	...
	private void calculateClassEntropy() {
		...
		for (Map.Entry<Value, int[]> entry : statistics[k].entrySet()) {
			numerators[i] = entry.getValue()[datasetService.intValue(k, entry
					.getKey())];
			total += numerators[i];
			if (max < numerators[i]) {
				max = numerators[i];
